...
func (bot *TradingBot) analyzeExistingPosition(position *Position) {
	...
	// ดึง OHLCV 200 แท่งสำหรับ AI analysis (1 ชั่วโมง)
	ohlcv, err := bot.gateClient.GetOHLCV(contract, "1h", 200)
	if err != nil {
...
func (bot *TradingBot) analyzeContract(contract string) bool {
	...
	// ดึง OHLCV 200 แท่ง (สำหรับ EMA200 filter)
	fmt.Printf("2️⃣ ดึงข้อมูล OHLCV (200 แท่ง, 1 ชั่วโมง)...\n")
	ohlcvFull, err := bot.gateClient.GetOHLCV(contract, "1h", 200)
	if err != nil {
	...
	if len(ohlcvFull) < 200 {
		fmt.Printf("❌ ข้อมูลไม่พอ (มี %d แท่ง, ต้องการ 200)\n", len(ohlcvFull))
		fmt.Printf("⏭️ ข้ามเหรียญนี้ (ข้อมูลไม่เพียงพอ)\n")
		return false
	}
	fmt.Printf("✅ ดึงข้อมูล OHLCV ได้ %d แท่ง\n", len(ohlcvFull))

	// ส่งข้อมูลครบทั้งหมด 200 แท่งให้ AI
	ohlcv := ohlcvFull
...
// DisplayDetailedPrompts แสดง AI prompts แบบละเอียด
func (bot *TradingBot) DisplayDetailedPrompts() {
	bot.aiClient.DisplayDetailedPrompts()
}
