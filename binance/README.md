# 🚀 Binance Trading Bot - ระบบเทรดอัตโนมัติ

ระบบเทรดอัตโนมัติที่ใช้ **Pure AI Trading System** สำหรับ Binance Futures Trading

## 📋 โครงสร้างโปรเจค

```
binance-trading-bot/
├── app.go                           # ไฟล์หลักของโปรแกรม
├── go.mod                          # Go module dependencies
├── go.sum                          # Go module checksums
├── internal/trading/               # โค้ดหลักของระบบเทรด
│   ├── bot.go                      # Trading Bot หลัก
│   ├── types.go                    # โครงสร้างข้อมูล
│   ├── binance_client.go           # Binance API client
│   ├── ai_client.go                # AI client (DeepSeek)
│   └── indicators.go               # Technical indicators
├── prompts/                        # Manual prompt files
│   ├── open_position_prompt.txt    # Prompt สำหรับเปิด position
│   └── close_position_prompt.txt   # Prompt สำหรับปิด position
├── test/                          # ไฟล์ทดสอบ
│   ├── connection_test.go         # ทดสอบการเชื่อมต่อ
│   ├── test_position.go           # ทดสอบการเปิด/ปิด position
│   └── ...                        # ไฟล์ทดสอบอื่นๆ
└── markdown/                      # เอกสารต้นฉบับ (ห้ามแก้ไข)
    ├── step.md                    # ข้อกำหนดระบบ
    └── sample.md                  # ตัวอย่างการทดสอบ
```

## 🔧 การติดตั้งและรัน

### 1. ตั้งค่า Environment Variables
สร้างไฟล์ `.env` ในโฟลเดอร์หลัก:
```env
BINANCE_API_KEY=your_binance_api_key_here
BINANCE_API_SECRET=your_binance_api_secret_here  
DEEPSEEK_API_KEY=your_deepseek_api_key_here
```

### 2. ติดตั้ง Dependencies
```bash
go mod tidy
```

### 3. รันโปรแกรม
```bash
go run app.go
```

## 🤖 คุณสมบัติหลัก

### ✨ Pure AI Trading System
- **Pure AI Mode**: AI ตัดสินใจทุกครั้ง ไม่พึ่งพา indicators
- **AI-Only Decision**: ดู action เท่านั้น (LONG/SHORT/HOLD) ไม่ดู confidence

### 📊 Technical Analysis
- **Data Source**: OHLCV 288 แท่ง (1 ชั่วโมง)
- **Volume Filter**: เฉพาะเหรียญที่มี volume > $100,000 ใน 24h
- **Time Frame**: 1H เท่านั้น

### 💰 Position Management
- **Position Size**: 15 USDT ต่อ position
- **Leverage**: 10x (isolated margin)
- **Risk-Reward Ratio**: ≥ 3.0
- **AI Decision**: LONG/SHORT/HOLD ไม่ดู confidence

### 🔄 Trading Loop
1. **LOOP1**: ตรวจสอบและจัดการ positions ที่เปิดอยู่ (ทุก 1 ชั่วโมง)
2. **LOOP2**: สแกนหาโอกาสใหม่ (volume filtering + EMA pre-filter เพื่อประหยัด AI calls)

## 📈 กฎการเทรด

### ✅ ข้อบังคับ
- ใช้ Pure AI System เท่านั้น
- Time Frame 1H เท่านั้น
- AI ตัดสินใจทั้งการเปิดและปิด position (ดู action เท่านั้น ไม่ดู confidence)
- Volume filtering เฉพาะเหรียญที่มี volume > $100,000 ใน 24h

### ❌ ข้อห้าม
- ห้ามเปิด position ซ้ำในเหรียญเดียวกัน
- ห้ามเปิด position เพิ่มหรือเปลี่ยนขนาด position
- ห้ามตั้ง Stop Loss/Take Profit อัตโนมัติ (เป็น AI-Only Mode)

## 🟢 Pure AI Strategy
**เงื่อนไข**: AI วิเคราะห์จากข้อมูล OHLCV ล้วนๆ
1. ดึงข้อมูล OHLCV 288 แท่ง (1 ชั่วโมง)
2. AI วิเคราะห์ price action, patterns, volume, momentum
3. ตัดสินใจ LONG/SHORT/HOLD ตาม AI recommendation

## 🔴 Pre-filtering System
**เงื่อนไข**: EMA pre-filter เพื่อประหยัด AI calls
1. คำนวณ EMA 20/50/100/200
2. เช็คเงื่อนไข LONG/SHORT ก่อน
3. เรียก AI เฉพาะเหรียญที่ผ่านเงื่อนไข

## 🧪 การทดสอบ

### ทดสอบการเชื่อมต่อ
```bash
cd test
go run connection_test.go
```

### ทดสอบการเปิด/ปิด Position
```bash
cd test  
go run test_position.go
```

## 🔐 ความปลอดภัย
- ใช้ isolated margin ลดความเสี่ยง
- Position size เล็ก (15 USDT)
- ไม่มี martingale หรือ grid
- AI ตัดสินใจทุกอย่าง ไม่มี hard rule

## 📊 การติดตามผล
- Balance และ PnL realtime
- Position status ทุกชั่วโมง  
- AI decision results (LONG/SHORT/HOLD)
- Volume filtering และ EMA pre-filtering results

## ⚠️ ข้อจำกัดที่ทราบ
- AI response อาจช้าหรือ timeout (มี retry mechanism)
- Balance ต้องมีอย่างน้อย 15 USDT ต่อ position
- Position Value: 15 USDT x 10x leverage = 150 USDT total exposure

---

**หมายเหตุ**: ระบบนี้เป็นการเทรดที่มีความเสี่ยง โปรดใช้เงินที่พร้อมเสียได้และศึกษาทำความเข้าใจก่อนใช้งาน
