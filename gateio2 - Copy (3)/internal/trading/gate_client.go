...
func (gc *GateClient) GetFuturesContracts() ([]string, error) {
	...
	// เพิ่มการกรองเหรียญที่มีประวัติมากกว่า 4 เดือน (เหรียญเก่า)
	fmt.Printf("📅 กำลังกรองเหรียญเก่า (มากกว่า 4 เดือน)...\n")
	oldCoinsContracts, err := gc.filterNewCoins(highVolumeContracts, 4)
	if err != nil {
		fmt.Printf("⚠️ ไม่สามารถกรองเหรียญเก่าได้: %v\n", err)
		return highVolumeContracts, nil // ใช้รายการเดิมถ้ากรองไม่ได้
	}

	fmt.Printf("⏳ กรองเหรียญเก่า (> 4 เดือน) แล้วได้ %d contracts\n", len(oldCoinsContracts))
	return oldCoinsContracts, nil
}

// filterNewCoins กรองเหรียญที่เข้าตลาดมากกว่า N เดือน (เหรียญเก่า) โดยใช้ daily timeframe
func (gc *GateClient) filterNewCoins(contracts []string, minMonths int) ([]string, error) {
	futuresApi := gc.client.FuturesApi
	var filteredContracts []string
	minDays := minMonths * 30 // แปลงเดือนเป็นวัน
	...
	for _, contract := range contracts {
		...
		if daysAvailable > minDays {
			fmt.Printf("   ⏳ %s: มีข้อมูล %d วัน (> %d วัน = %d เดือน) ✅\n",
				contract, daysAvailable, minDays, minMonths)
