# 📋 Gate.io Trading Bot - Pure AI Trading System

## 🎯 ภาพรวมระบบ
ระบบเทรดอัตโนมัติที่ใช้ AI เป็นหลักในการตัดสินใจเปิด/ปิด positions บน Gate.io Futures โดยไม่พึ่งพา SuperTrend หรือ EMA indicators

## ⚙️ กฎการพัฒนาและบำรุงรักษา
- ใช้ Golang เป็นหลัก
- เป็น Unicode ทั้งหมด รวมถึงข้อความ output และ source code
...
1. **ดึงข้อมูล**: OHLCV 144 แท่งล่าสุด (1 ชั่วโมง)
2. **ส่งให้ AI**: วิเคราะห์ทิศทาง LONG/SHORT/HOLD

### 🔴 AI Decision Only  
**เงื่อนไข:** AI ตัดสินใจทุกครั้ง
1. **รับข้อมูล**: OHLCV 144 แท่ง (1 ชั่วโมง)
...
2. ดึง OHLCV 144 แท่งล่าสุด (1 ชั่วโมง) สำหรับแต่ละ position
...
6. ดึงรายชื่อเหรียญจาก Gate.io Futures (USDT pairs, volume > $100,000 ใน 24h)

### LOOP2: สแกนหาโอกาสใหม่
1. วิเคราะห์เหรียญทีละตัวด้วย AI (เฉพาะเหรียญที่มี volume > $100,000 ใน 24h)
   - ดึง OHLCV 144 แท่ง (1 ชั่วโมง)
   - ตรวจสอบ EMA30 Touch condition (แท่งเทียน -1 ทับเส้น EMA30)
   - ส่งข้อมูลให้ AI วิเคราะห์ (เฉพาะที่ผ่านเงื่อนไข)
   - AI ตัดสินใจ LONG/SHORT/HOLD

2. **เงื่อนไขการเปิด position (EMA30 Filter + AI Mode)**:
   - **EMA30 Touch**: แท่งเทียน -1 timeframe ทับเส้น EMA30 พอดี
   - **AI Decision**: LONG หรือ SHORT (ไม่ดู confidence)
...
1. **เปิด Position (Pure AI Mode)**:
   - รับข้อมูล: OHLCV 144 แท่ง (1 ชั่วโมง)
...
2. **ปิด Position (ใช้ AI เท่านั้น)**:
   - รับข้อมูล: OHLCV 144 แท่ง (1 ชั่วโมง) + position data
