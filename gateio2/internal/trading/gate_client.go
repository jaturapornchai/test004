...
// GetFuturesContracts ดึงรายชื่อ contracts ที่มี volume > $100,000 ใน 24h
func (gc *GateClient) GetFuturesContracts() ([]string, error) {
	...
	for _, contract := range usdtContracts {
		if ticker, exists := tickerMap[contract]; exists {
			...
			if err == nil && volumeUSD > 100000 { // > $100,000
				highVolumeContracts = append(highVolumeContracts, contract)
			}
		}
	}

	fmt.Printf("💰 กรอง volume > $100,000 แล้วได้ %d contracts\n", len(highVolumeContracts))

	// เพิ่มการกรองเหรียญที่มีประวัติน้อยกว่าหรือเท่ากับ 4 เดือน (เหรียญใหม่)
	fmt.Printf("📅 กำลังกรองเหรียญใหม่ (น้อยกว่าหรือเท่ากับ 4 เดือน)...\n")
	newCoinsContracts, err := gc.filterNewCoins(highVolumeContracts, 4)
	if err != nil {
		fmt.Printf("⚠️ ไม่สามารถกรองเหรียญใหม่ได้: %v\n", err)
		return highVolumeContracts, nil // ใช้รายการเดิมถ้ากรองไม่ได้
	}

	fmt.Printf("🆕 กรองเหรียญใหม่ (≤ 4 เดือน) แล้วได้ %d contracts\n", len(newCoinsContracts))
	return newCoinsContracts, nil
}

// filterNewCoins กรองเหรียญที่เข้าตลาดน้อยกว่าหรือเท่ากับ N เดือน (เหรียญใหม่) โดยใช้ daily timeframe
func (gc *GateClient) filterNewCoins(contracts []string, maxMonths int) ([]string, error) {
	futuresApi := gc.client.FuturesApi
	var filteredContracts []string
	maxDays := maxMonths * 30 // แปลงเดือนเป็นวัน

	for _, contract := range contracts {
		// ดึงข้อมูล daily candles (ขอ 150 วัน เผื่อ ≈ 5 เดือน)
		candles, _, err := futuresApi.ListFuturesCandlesticks(gc.ctx, "usdt", contract, &gateapi.ListFuturesCandlesticksOpts{
			Interval: optional.NewString("1d"), // daily timeframe
			Limit:    optional.NewInt32(150),   // ขอ 150 วัน
		})

		if err != nil {
			// ถ้าดึงข้อมูลไม่ได้ ข้ามเหรียนนี้
			fmt.Printf("   ❌ %s: ไม่สามารถดึงข้อมูล daily ได้ - ข้าม\n", contract)
			continue
		}

		// นับจำนวนวันที่มีข้อมูล
		daysAvailable := len(candles)

		if daysAvailable <= maxDays {
			fmt.Printf("   🆕 %s: มีข้อมูล %d วัน (≤ %d วัน = %d เดือน) ✅\n",
				contract, daysAvailable, maxDays, maxMonths)
